 ...
 */
const monthDaysMappings: number[][][] = yearMonthDaysMapping.map(
  (yearMappings: number[]) => {
    let daySum = 0;
 ...
 */
const yearDaysMapping: number[][] = yearMonthDaysMapping.map(
  (yearMappings: number[]) => {
...
export function mapDaysToDate(daysPassed: number): IYearMonthDate {
  if (daysPassed < MIN_DAY || daysPassed > MAX_DAY) {
    throw new Error(
      `The epoch difference is not within the boundaries ${MIN_DAY} - ${MAX_DAY}`
    );
  }

  const yearIndex = yearDaysMapping.findIndex(
    (year) =>
      daysPassed > year[COMPLETED_DAYS] &&
      daysPassed <= year[COMPLETED_DAYS] + year[TOTAL_DAYS]
  );
  const monthRemainder =
    daysPassed - yearDaysMapping[yearIndex][COMPLETED_DAYS];
  ...
  const date =
    monthRemainder - monthDaysMappings[yearIndex][monthIndex][COMPLETED_DAYS];
...
export function findPassedDaysAD(year: number, month: number, date: number) {
  const timeDiff = Math.abs(
    Date.UTC(year, month, date) -
      Date.UTC(beginEnglish.year, beginEnglish.month, beginEnglish.date)
  );
...
export function convertToAD(bsDateObject: IYearMonthDate): IAdBs {
  try {
    const daysPassed = findPassedDays(
      bsDateObject.year,
      bsDateObject.month,
      bsDateObject.date
    );
...
export function format(
  ...
  return stringFormat
    .replace(
      /((\\[MDYd])|D{1,2}|M{1,4}|Y{2,4}|d{1,3})/g,
      (match, _, matchedString) => {
        switch (match) {
          case "D":
            return mapLanguageNumber(bsDate.date.toString(), language);
          case "DD":
            return mapLanguageNumber(
              bsDate.date.toString().padStart(2, "0"),
              language
            );
          case "M":
          ...
          case "MM":
            return mapLanguageNumber(
              (bsDate.month + 1).toString().padStart(2, "0"),
              language
            );
          case "MMM":
          ...
          case "YY":
            return mapLanguageNumber(
              bsDate.year.toString().slice(-2),
              language
            );
          case "YYY":
            return mapLanguageNumber(
              bsDate.year.toString().slice(-3),
              language
            );
          case "YYYY":
